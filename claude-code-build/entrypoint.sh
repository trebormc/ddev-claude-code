...
# --- 1. Copy CLAUDE.md from agents-sync volume to USER level ---
# ~/.claude/CLAUDE.md is the user-level memory: Claude Code loads it for every
# session IN ADDITION to the project's own CLAUDE.md, which stays untouched so
# each Drupal project can keep its own project-specific instructions.
